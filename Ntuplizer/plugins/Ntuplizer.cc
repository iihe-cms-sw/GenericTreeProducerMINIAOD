...
#include "GenericTreeProducerMINIAOD/Ntuplizer/interface/Tools.h"
...
//#include "GenericTreeProducerMINIAOD/Ntuplizer/python/RochesterCorrections/RoccoR.h"
#include "GenericTreeProducerMINIAOD/Ntuplizer/interface/RoccoR.h"
...
class Ntuplizer : public edm::one::EDAnalyzer<edm::one::SharedResources,edm::one::WatchRuns>  {
  ...
  Float_t l1muqual12_leadingpt;
  Float_t l1muqual12_subleadingpt;
  Float_t l1jet_leadingpt;
...
//  jecsToken_(esConsumes(edm::ESInputTag("","AK4PFchs"))),
  ...
  RunEra_(iConfig.getParameter<string>("RunEra")),
...
Ntuplizer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  */
  ...
  if( !trigResults.failedToGet() ) {
	...
	if(TrigPath.Contains("HLT_ZeroBias_v")) HLT_ZeroBias=true;
	if(TrigPath.Contains("HLT_Physics_v")) HLT_Physics=true;
	if(TrigPath.Contains("HLT_EphemeralZeroBias_v"))HLT_EphemeralZeroBias=true;
	if(TrigPath.Contains("HLT_EphemeralPhysics_v")) HLT_EphemeralPhysics=true;
      }
    }
  }
...
//  cout << "get bit for L1_ZeroBias: " << thebit << ", "<<lol<<endl;
  /*
  for(int i = 0; i < 20; i++){
  ...
  int thebit_zb = -1;
  l1GtUtils_->getAlgBitFromName("L1_ZeroBias",thebit_zb);
  passzb_bx0 = passL1_Initial_bx0[thebit_zb];
  passzb_bx1 = passL1_Initial_bxplus1[thebit_zb];
  passzb_bx2 = passL1_Initial_bxplus2[thebit_zb];
  ...
  l1muqual12_leadingpt = 0;
  l1muqual12_subleadingpt = 0; 
  ...
  for(int i = l1muoncoll->getFirstBX() ; i<= l1muoncoll->getLastBX() ;i++){
    for( l1t::MuonBxCollection::const_iterator l1muonit= l1muoncoll->begin(i); l1muonit != l1muoncoll->end(i) ; ++l1muonit){
      ...
      if(i==0&& l1muonit->hwQual()>=12){
	if(l1muonit->pt() > l1muqual12_leadingpt){
	  l1muqual12_subleadingpt = l1muqual12_leadingpt ; l1muqual12_leadingpt = l1muonit->pt(); 
	}
	else if(l1muonit->pt() > l1muqual12_subleadingpt){ l1muqual12_subleadingpt = l1muonit->pt();}
      }
  ...
  for(int i = l1egcoll->getFirstBX() ; i<= l1egcoll->getLastBX() ;i++){
	...
	else if(l1egit->pt() > l1eg_subleadingpt){ l1eg_subleadingpt = l1egit->pt();}
      }
    }
  }
  ...
  for(int i = l1jetcoll->getFirstBX() ; i<= l1jetcoll->getLastBX() ;i++){
    for( l1t::JetBxCollection::const_iterator l1jetit= l1jetcoll->begin(i); l1jetit != l1jetcoll->end(i) ; ++l1jetit){
      ...
      if(i==0){
	if(l1jetit->pt() > l1jet_leadingpt){
	  l1jet_subleadingpt = l1jet_leadingpt ; l1jet_leadingpt = l1jetit->pt(); 
	}
	else if(l1jetit->pt() > l1jet_subleadingpt){ l1jet_subleadingpt = l1jetit->pt();}
      }
    }
  }


  edm::Handle<l1t::EtSumBxCollection> l1sumcoll;
  iEvent.getByToken(l1EtSumToken_ , l1sumcoll);
  for (int i = l1sumcoll->getFirstBX(); i <= l1sumcoll->getLastBX(); ++i) {
    _L1etsum_bx.push_back( i);
    for (l1t::EtSumBxCollection::const_iterator it = l1sumcoll->begin(i); it != l1sumcoll->end(i); it++) {
      if(l1t::EtSum::EtSumType::kMissingEtHF == it->getType()){
	_L1etsum_etmhf.push_back( it->pt());
	_L1etsum_etmhf_phi.push_back( it->phi());
      }
      if(l1t::EtSum::EtSumType::kMissingHtHF == it->getType()){ 
	_L1etsum_mht.push_back( it->pt());
	_L1etsum_mht_phi.push_back( it->phi());
      }
  ...
  if(Skim_ == "L1Studies_EphemeralHLTPhysics" || Skim_ == "L1Studies_EphemeralZeroBias"){
    ...
    if((HLT_ZeroBias||HLT_Physics||HLT_EphemeralZeroBias||HLT_EphemeralPhysics)&&SaveTree_)outputTree->Fill(); 
    return;
  }
  ...
  */
  ...
  if (lhe_handle.isValid()){ 
    ...
    ROOT::Math::PxPyPzEVector p4neutrinos_;
    ...
    for (unsigned i = 0; i < lheParticles.size(); ++i) {
      ...
      int abspdgid = fabs(lhe_handle->hepeup().IDUP[i]);

      if(abspdgid==12||abspdgid==14||abspdgid==16) p4neutrinos_ += cand_;
...
Ntuplizer::beginJob()
{
  ...
  if(Skim_ == "L1Studies_EphemeralHLTPhysics" || Skim_ == "L1Studies_EphemeralZeroBias"){
    ...
    outputTree->Branch("passzb_bx0",&passzb_bx0,"passzb_bx0/O");
    outputTree->Branch("passzb_bx1",&passzb_bx1,"passzb_bx1/O");
    outputTree->Branch("passzb_bx2",&passzb_bx2,"passzb_bx2/O");
    ...
    outputTree->Branch("l1jet_leadingpt", &l1jet_leadingpt, "l1jet_leadingpt/F");
    ...
    outputTree->Branch("HLT_ZeroBias",&HLT_ZeroBias,"HLT_ZeroBias/O");
    outputTree->Branch("HLT_Physics",&HLT_Physics,"HLT_Physics/O");
    outputTree->Branch("HLT_EphemeralZeroBias",&HLT_EphemeralZeroBias,"HLT_EphemeralZeroBias/O");
    outputTree->Branch("HLT_EphemeralPhysics",&HLT_EphemeralPhysics,"HLT_EphemeralPhysics/O");
  ...
  if(Skim_=="ZToEEorMuMu" || Skim_=="Dilepton" || Skim_=="DileptonInfo" || Skim_=="ZHFJet" || Skim_==""){
    outputTree->Branch("_mll", &_mll, "_mll/F");
    outputTree->Branch("_ptll", &_ptll, "_ptll/F");
    outputTree->Branch("_pzll", &_pzll, "_pzll/F");
    outputTree->Branch("_yll", &_yll, "_yll/F");
  ...
  if(IsMC_){
    ...
    outputTree->Branch("_genMassNus",&_genMassNus,"_genMassNus/F");
...
bool Ntuplizer::GetMETFilterDecision(const edm::Event& iEvent,edm::Handle<TriggerResults> METFilterResults, TString studiedfilter){
...
bool Ntuplizer::GetIdxFilterDecision(int it){
...
TString Ntuplizer::GetIdxFilterName(int it){
  if(it==idx_Flag_goodVertices)return "Flag_goodVertices";
  else if(it== idx_Flag_globalTightHalo2016Filter)return "Flag_globalTightHalo2016Filter";
  else if(it== idx_Flag_globalSuperTightHalo2016Filter)return "Flag_globalSuperTightHalo2016Filter";
  else if(it== idx_Flag_HBHENoiseFilter)return "Flag_HBHENoiseFilter";
...
void Ntuplizer::InitandClearStuff(){
  _mll=0;
  _ptll=0;
  _pzll=0;
  _yll=0;
  ...
  _L1etsum_etmhf.clear();
  _L1etsum_etmhf_phi.clear();
  _L1etsum_mht.clear();
  ...
  HLT_ZeroBias=false;
  HLT_Physics=false;
  HLT_EphemeralZeroBias=false;
  HLT_EphemeralPhysics=false;
  
  HLT_PixelClusters_WP1=false;
  HLT_PixelClusters_WP2=false;
  HLT_PixelClusters_WP2_split=false;
...
bool Ntuplizer::PassSkim(){
  ...
  if(Skim_=="ZToEEorMuMu" || Skim_=="Dilepton" || Skim_=="L1Study_ZToMuMu" || Skim_=="L1Study_ZToEE"){
    ...
    if (!PassTrigger4L()) return false;
    //First check the dataset for data:
...
Ntuplizer::CalcDileptonInfo(const int& i, const int& j, Float_t & themass, Float_t & theptll, Float_t & thepzll,  Float_t & theyll, Float_t & thephill, Float_t & thedphill, Float_t & thecosthll){
 ...
 void
Ntuplizer::CalcDileptonInfoGen(const int& i, const int& j, Float_t & themass, Float_t & theptll, Float_t & thepzll,  Float_t & theyll,Float_t & thephill,  Float_t & thedphill, Float_t & thecosthll){
...
void Ntuplizer::CalcFourLeptonInfo(Float_t & themass, Float_t & theptll, Float_t & thepzll,  Float_t & theyll){
...
bool Ntuplizer::PassTriggerLeg(std::string triggerlegstring, std::string triggerlegstringalt,  const pat::Muon *muonit, const edm::Event& iEvent ){
...
bool Ntuplizer::PassTriggerLeg(std::string triggerlegstring, std::string triggerlegstringalt,const pat::Electron *eleit, const edm::Event& iEvent ){
...
bool Ntuplizer::PassTriggerLeg(std::string triggerlegstring, std::string triggerlegstringalt,const pat::Photon *photonit, const edm::Event& iEvent ){
...
bool Ntuplizer::PassTriggerLeg(std::string triggerlegstring, std::string triggerlegstringalt,const pat::Jet *jetit, const edm::Event& iEvent ){
...
int  Ntuplizer::GetRecoIdx(const reco::GenJet * genjet , vector <Float_t> recojetpt,  vector <Float_t> recojeteta, vector <Float_t> recojetphi, vector <Float_t> recojetptgen ){
...
bool Ntuplizer::IsTauCleaned(const pat::Jet *iJ){
...
bool Ntuplizer::IsLeptonPhotonCleaned(const pat::Jet *iJ){
  for(unsigned int i = 0 ; i < _lPt.size(); i++){
...
bool Ntuplizer::IsLeptonPhotonCleaned(const reco::CaloJet *iJ){
  for(unsigned int i = 0 ; i < _lPt.size(); i++){
    double dr = deltaR( _lEta[i],_lPhi[i],iJ->eta(),iJ->phi()); 
    if(dr<0.3) return false; 
  }
...
bool Ntuplizer::PassJetPreselection(const pat::Jet * iJ, double genjetpt, bool ispuppi){
...
bool Ntuplizer::PassJetPreselection(const reco::CaloJet * iJ, double genjetpt){
  if(Skim_=="MCJECs" &&  genjetpt<5) return false;
  else if(  iJ->pt()< JetPtCut_) return false;
  return true;
...
bool Ntuplizer::PassTrigger4L(){
  
  
  bool passtriggers[5] = { 
    (HLT_Mu23_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL_DZ||HLT_Mu8_TrkIsoVVL_Ele23_CaloIdL_TrackIdL_IsoVL_DZ||HLT_Mu23_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL||HLT_Mu8_TrkIsoVVL_Ele23_CaloIdL_TrackIdL_IsoVL),
    (HLT_IsoMu24||HLT_IsoTkMu24||HLT_IsoMu27), 
    (HLT_TkMu17_TrkIsoVVL_TkMu8_TrkIsoVVL_DZ||HLT_Mu17_TrkIsoVVL_TkMu8_TrkIsoVVL_DZ||HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL||HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ||HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ_Mass3p8),
  ...
  if(RunEra_.find("MC") !=std::string::npos) return passtriggers[0] || passtriggers[1] || passtriggers[2] || passtriggers[3] || passtriggers[4];
  
  
  
  if(_runNb>=271036 &&_runNb<314472){ 
    if(Dataset_.find("MuonEG") !=std::string::npos) return passtriggers[0];
    if(Dataset_.find("SingleMuon") !=std::string::npos) return !passtriggers[0] && passtriggers[1];
    if(Dataset_.find("DoubleMuon") !=std::string::npos) return !passtriggers[0] && !passtriggers[1] && passtriggers[2];
    if(Dataset_.find("SingleElectron") !=std::string::npos) return !passtriggers[0] && !passtriggers[1] && !passtriggers[2] && passtriggers[3];
    if(Dataset_.find("DoubleEG") !=std::string::npos) return !passtriggers[0] && !passtriggers[1] && !passtriggers[2] && !passtriggers[3] && passtriggers[4];
  }
  
  //2018 or 2022B/C runs <356426  
  if(_runNb>=314472 &&_runNb<356426){
    if(Dataset_.find("MuonEG") !=std::string::npos) return passtriggers[0];
    if(Dataset_.find("SingleMuon") !=std::string::npos) return !passtriggers[0] && passtriggers[1];
    if(Dataset_.find("DoubleMuon") !=std::string::npos) return !passtriggers[0] && !passtriggers[1] && passtriggers[2];
    if(Dataset_.find("EGamma") !=std::string::npos) return !passtriggers[0] && !passtriggers[1] && !passtriggers[2] && (passtriggers[3] || passtriggers[4]);
  }
  
  //2022C run >=356426 (this is when SingleMuon and DoubleMuon were merged)  or 2022DEFG 
  if(_runNb>=356426 &&_runNb<362761){
    if(Dataset_.find("MuonEG") !=std::string::npos) return passtriggers[0];
    if(Dataset_.find("Muon") !=std::string::npos) return !passtriggers[0] && (passtriggers[1] || passtriggers[2]);
    if(Dataset_.find("EGamma") !=std::string::npos) return !passtriggers[0] && !passtriggers[1] && !passtriggers[2] && (passtriggers[3] || passtriggers[4]);
  }
